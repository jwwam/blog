...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static byte[] readFile(String path)
	{
		...
		{
			System.out.println(path+" 文件读取失败！");
		}
		finally
		{
			try
			...
			{
				System.out.println( path+" 读取文件，关闭失败！");
			}
		}
		return null;
	}
	...
	/** 写入文件 **/
	public static boolean wirteFile(String path,String fileName, byte[] data) {
		...
		try {
			...
			System.out.println(path + " 文件写入失败！");
		} finally {
			try {
				if (outputStream != null)
					outputStream.close();
			} catch (Exception ex) {
				System.out.println( path + " 写入文件，关闭失败！");
			}
		}
		return false;
	}
